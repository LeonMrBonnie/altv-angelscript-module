...
struct SystemCall
{
    ...
    SystemCall(
      Processor& CPU, FloatingPointUnit& FPU, std::function<void(JumpType, bool)> ConditionalReturn, asDWORD* const& bytecode, unsigned JitFlags)
        : cpu(CPU), fpu(FPU), returnHandler(ConditionalReturn), pOp(bytecode), flags(0)
    {
        if((JitFlags & JIT_SYSCALL_NO_ERRORS) != 0) flags |= SC_Safe;
        if((JitFlags & JIT_NO_SUSPEND) != 0) flags |= SC_NoSuspend;
        if((JitFlags & JIT_SYSCALL_FPU_NORESET) != 0) flags |= SC_FastFPU;
    }
...
{
    if(activeJumpTable) delete[] activeJumpTable;
    delete lock;
}
...
int asCJITCompiler::CompileFunction(asIScriptFunction* function, asJITFunction* output)
{
    ...
    *output              = activePage->getFunctionPointer<asJITFunction>();
    ...
    Register pax(cpu, EAX, pBits), pbx(cpu, EBX, pBits), pcx(cpu, ECX, pBits), pdx(cpu, EDX, pBits), esp(cpu, ESP, pBits), pdi(cpu, R12, pBits),
      esi(cpu, R13, pBits);
    ...
    Register al(cpu, EAX, 8), bl(cpu, EBX, 8), cl(cpu, ECX, 8), dl(cpu, EDX, 8);
    // Pointer-sized registers
    Register pax(cpu, EAX, pBits), pbx(cpu, EBX, pBits), pcx(cpu, ECX, pBits), pdx(cpu, EDX, pBits), esp(cpu, ESP, pBits), pdi(cpu, EDI, pBits),
      esi(cpu, ESI, pBits);
    ...
    while(pOp < end)
    {
        ...
        if(cpu.op > activePage->getActivePage() + activePage->getFreeSize()) throw "Page exceeded...";
        ...
        if(activeSwitch)
        {
            ...
            if(--activeSwitch->remaining == 0) activeSwitch = 0;
        }
        ...
        *pdx                 = (void*)beg;
        ...
        if(pNextOp < end && jumpTable[pNextOp - start] == nullptr)
        {
            ...
            if(pThirdOp < end && jumpTable[pThirdOp - start] == nullptr)
            {
                ...
                {
                    case asBC_SetV8:
                        if(thirdOp == asBC_CpyVtoV8 && (nextOp == asBC_ADDd || nextOp == asBC_DIVd || nextOp == asBC_SUBd || nextOp == asBC_MULd))
                        {
                            if(asBC_SWORDARG0(pOp) != asBC_SWORDARG2(pNextOp) || asBC_SWORDARG0(pOp) != asBC_SWORDARG0(pNextOp)) break;
                            ...
                            switch(nextOp)
                            {
                                case asBC_ADDd: fpu.add_double(doubleConstant); break;
                                case asBC_SUBd: fpu.sub_double(doubleConstant); break;
                                case asBC_MULd: fpu.mult_double(doubleConstant); break;
                                case asBC_DIVd: fpu.div_double(doubleConstant); break;
                            }
                        ...
                        break;
                    case asBC_SetV4:
                        if(nextOp == asBC_SetV4 && thirdOp == asBC_SetV4 && asBC_DWORDARG(pOp) == asBC_DWORDARG(pNextOp) &&
                           asBC_DWORDARG(pNextOp) == asBC_DWORDARG(pThirdOp))
                        {
                            // Optimize intializing 3 variables to the same value (often 0)
                            if(asBC_DWORDARG(pOp) == 0) eax ^= eax;
                            else
                    ...
                    case asBC_PshVPtr:
                        ...
                        if(nextOp == asBC_ADDSi && thirdOp == asBC_RDSPtr)
                        {
                            ...
                            if(reservedPushBytes != 0) reservedPushBytes = 0;
                            else
                                esi -= sizeof(void*);
            ...
            switch(op)
            {
                ...
                case asBC_RDR4:
                    if(nextOp == asBC_PshV4 && asBC_SWORDARG0(pOp) == asBC_SWORDARG0(pNextOp))
                    {
                        ...
                        *esi + reservedPushBytes = eax;
                        if(reservedPushBytes == 0) nextEAX = EAX_Stack;
                ...
                case asBC_CMPIu:
                {
                    ...
                    {
                        case asBC_JZ:
                        case asBC_JLowZ: jump = Equal; break;
                        case asBC_JNZ:
                        case asBC_JLowNZ: jump = NotEqual; break;
                        case asBC_JS: jump = isUnsigned ? Below : Sign; break;
                        case asBC_JNS: jump = isUnsigned ? NotBelow : NotSign; break;
                        case asBC_JP: jump = isUnsigned ? Above : Greater; break;
                    ...
                    {
                        eax = *edi - offset0;
                        if(op == asBC_CMPIi || op == asBC_CMPIu) eax == asBC_DWORDARG(pOp);
                        else
                            eax == *edi - offset1;
        ...
        {
            case asBC_JitEntry:
                if(!firstJitEntry) firstJitEntry = (void*)cpu.op;
...
#define pushPrep(use)                                         \
    ...
    reservedPushBytes -= use;
            ...
            case asBC_NOT:
            {
                if(currentEAX != EAX_Offset + offset0) al = as<byte>(*edi - offset0);
                al &= al;
                al.setIf(Zero);
                eax.copy_zeroing(al);
                *edi - offset0 = eax;
            ...
            break;
            case asBC_JMP: do_jump(Jump); break;
            ...
            case asBC_TNP:
                bl == 0;
                ebx.setIf(LessOrEqual);
                ebx.copy_zeroing(ebx);
                break;

            case asBC_NEGi: -(*edi - offset0); break;
            case asBC_NEGf:
            ...
            case asBC_NEGd:
                ...
                break;
            case asBC_INCi16: ++as<short>(*ebx); break;
            case asBC_INCi8: ++as<char>(*ebx); break;
            case asBC_DECi16: --as<short>(*ebx); break;
            case asBC_DECi8: --as<char>(*ebx); break;
            case asBC_INCi: ++*ebx; break;
            case asBC_DECi: --*ebx; break;
            case asBC_INCf:
                fpu.load_const_1();
                fpu.add_float(*ebx);
                fpu.store_float(*ebx);
            ...
            case asBC_DECd:
                ...
                fpu.store_double(*ebx);
                break;
            case asBC_IncVi: ++(*edi - offset0); break;
            case asBC_DecVi: --(*edi - offset0); break;
            case asBC_BNOT: ~(*edi - offset0); break;
            case asBC_BAND:
                if(currentEAX != EAX_Offset + offset1) eax = *edi - offset1;
                eax &= *edi - offset2;
                *edi - offset0 = eax;
                nextEAX        = EAX_Offset + offset0;
                break;
            ...
            case asBC_COPY:
            {
                ...
                if(bytes <= 128)
                {
                    ...
                    else
                    {
                        ...
                        if(unroll)
                        {
                            for(unsigned i = 1; i < iterations; ++i) cpu.string_copy(copySize);
                        }
                        else
                        {
                            cpu.loop(loop);
                ...
                // ERR
                cpu.end_short_jump(test1);
                cpu.end_short_jump(test2);
    ...
    case asBC_STR:
            ...
            case asBC_CALLBND:
            {
                ...
                if(flags & JIT_NO_SCRIPT_CALLS)
                {
                    ...
                    cpu.call_stdcall((void*)callBoundFunction, "rc", &arg0, (unsigned int)asBC_INTARG(pOp));
                    pax &= pax;
            ...
            case asBC_ALLOC:
            {
                ...
                {
                    cpu.call_stdcall((void*)engineAlloc, "pp", (asCScriptEngine*)function->GetEngine(), objType);
            ...
            case asBC_FREE:
            {
                ...
                {  // Only do FREE on non-reference types, or reference types without fake reference counting
                    ...
                    if(beh->release)
                    {
                        ...
                        if((flags & JIT_FAST_REFCOUNT) != 0) callFlags |= SC_NoSuspend | SC_Safe;
                    ...
                    else if(beh->destruct)
                    {
                        ...
                        cpu.call_stdcall((void*)engineDestroyFree,
                                         "prp",
                    ...
                    else if(objType->flags & asOBJ_LIST_PATTERN)
                    {
                        ...
                        cpu.call_stdcall((void*)engineListFree, "ppr", (asCScriptEngine*)function->GetEngine(), objType, &arg1);
                    }
                    else
                    {
                        // Copy over registers to the vm in case the called functions observe the call stack
                        ...
                        cpu.call_stdcall((void*)engineFree, "pr", (asCScriptEngine*)function->GetEngine(), &arg1);
                    }
            ...
            case asBC_SetV2:
            case asBC_SetV4: *edi - offset0 = asBC_DWORDARG(pOp); break;
            case asBC_SetV8:
            {
#ifdef JIT_64
            ...
            case asBC_CpyVtoV4: as<asDWORD>(*edi - offset0).direct_copy(as<asDWORD>(*edi - offset1), eax); break;
            case asBC_CpyVtoV8: as<long long>(*edi - offset0).direct_copy(as<long long>(*edi - offset1), eax); break;

            case asBC_CpyVtoR4: ebx = *edi - offset0; break;
            case asBC_CpyVtoR8:
#ifdef JIT_64
            ...
            case asBC_CpyRtoV4: as<unsigned>(*edi - offset0) = as<unsigned>(ebx); break;
            case asBC_CpyRtoV8:
#ifdef JIT_64
            ...
            case asBC_RDR4:
                ...
                nextEAX = EAX_Offset + offset0;
                break;
            case asBC_RDR8: as<asQWORD>(*edi - offset0).direct_copy(as<asQWORD>(*ebx), eax); break;
            case asBC_LDG: pbx = (void*)asBC_PTRARG(pOp); break;
            case asBC_LDV: pbx.copy_address(*edi - offset0); break;
            // case asBC_PGA: //All pushes are handled above, near asBC_PshC4
            case asBC_CmpPtr:
            {
                if(currentEAX != EAX_Offset + offset0) pax = as<void*>(*edi - offset0);
            ...
            case asBC_swTOi:
                ...
                *edi - offset0 = eax;
                break;
            case asBC_ubTOi: *edi - offset0 &= 0xff; break;
            case asBC_uwTOi: *edi - offset0 &= 0xffff; break;
            case asBC_ADDi:
                if(currentEAX != EAX_Offset + offset1) eax = *edi - offset1;
                eax += *edi - offset2;
                *edi - offset0 = eax;
            ...
            case asBC_MULIf:
                ...
                fpu.store_float(*edi - offset0);
                break;
            case asBC_SetG4: MemAddress(cpu, (void*)asBC_PTRARG(pOp)) = asBC_DWORDARG(pOp + AS_PTR_SIZE); break;
            case asBC_ChkRefS:
                // Return if *(*esi) == 0
                if(currentEAX != EAX_Stack) pax = as<void*>(*esi);
                eax = as<int>(*pax);
            ...
            case asBC_ChkNullV:
                ...
                if(currentEAX != EAX_Offset + offset0) eax = *edi - offset0;
                eax &= eax;
                ReturnCondition(Zero);
                break;
            case asBC_CALLINTF:
            ...
            case asBC_iTOb: *edi - offset0 &= 0xff; break;
            case asBC_iTOw: *edi - offset0 &= 0xffff; break;
        ...
        #define cast(f, t)                                                      \
            {                                                                   \
                ...
                {                                                               \
                    arg0.copy_address(*edi - offset1);                          \
                    cpu.call_stdcall(func, "rr", &arg0, &arg1);                 \
                }                                                               \
                else                                                            \
                {                                                               \
                    arg0 = arg1;                                                \
        ...
        #define cast(f, t)                                                      \
            {                                                                   \
                ...
                {                                                               \
                    arg0.copy_address(*edi - offset1);                          \
                    cpu.call_cdecl(func, "rr", &arg0, &arg1);                   \
                }                                                               \
                else                                                            \
                {                                                               \
                    arg0 = arg1;                                                \
        ...
        #define cast(f, t)                                                   \
            {                                                                \
                ...
                {                                                            \
                    ...
                    cpu.call_stdcall(func, "rr", &pcx, &pax);                \
                }                                                            \
                else                                                         \
        ...
        #define cast(f, t)                                                   \
            {                                                                \
                ...
                {                                                            \
                    pcx.copy_address(*edi - offset1);                        \
                    cpu.call_cdecl(func, "rr", &pcx, &pax);                  \
                }                                                            \
                else                                                         \
            ...
            case asBC_iTOf:
                ...
                fpu.store_float(*edi - offset0);
                break;
            case asBC_fTOi: cast(float, int); break;
            case asBC_uTOf: cast(unsigned, float); break;
            case asBC_fTOu: cast(float, unsigned); break;
            case asBC_dTOi: cast(double, int); break;
            case asBC_dTOu: cast(double, unsigned); break;
            case asBC_dTOf:
                fpu.load_double(*edi - offset1);
                fpu.store_float(*edi - offset0);
                break;
            case asBC_iTOd:
                ...
                fpu.store_double(*edi - offset0);
                break;
            case asBC_uTOd: cast(unsigned, double); break;
            case asBC_fTOd:
                fpu.load_float(*edi - offset1);
                fpu.store_double(*edi - offset0);
                break;
            case asBC_i64TOi: cast(long long, int) break;
            case asBC_uTOi64: cast(unsigned int, long long) break;
            case asBC_iTOi64: cast(int, long long) break;
            case asBC_fTOi64: cast(float, long long) break;
            case asBC_fTOu64: cast(float, unsigned long long) break;
            case asBC_i64TOf: cast(long long, float) break;
            case asBC_u64TOf: cast(unsigned long long, float) break;
            case asBC_dTOi64: cast(double, long long) break;
            case asBC_dTOu64: cast(double, unsigned long long) break;
            case asBC_i64TOd: cast(long long, double) break;
            case asBC_u64TOd: cast(unsigned long long, double) break;

            case asBC_NEGi64: -as<long long>(*edi - offset0); break;
            case asBC_INCi64: ++as<long long>(*ebx); break;
            case asBC_DECi64: --as<long long>(*ebx); break;
            case asBC_BNOT64: ~as<long long>(*edi - offset0); break;
            case asBC_ADDi64:
            {
#ifdef JIT_64
            ...
            case asBC_ChkNullS:
            {
                if(asBC_WORDARG0(pOp) != 0 && currentEAX != EAX_Stack) eax = *esi + (asBC_WORDARG0(pOp) * sizeof(asDWORD));
                eax &= eax;
            ...
            break;
            case asBC_LoadVObjR: pbx.copy_address(*edi + (asBC_SWORDARG1(pOp) - offset0)); break;
            case asBC_AllocMem:
            {
    ...
    if(waitingForEntry == false) Return(true);
...
void asCJITCompiler::finalizePages()
{
    ...
    for(auto page = pages.begin(); page != pages.end(); ++page)
        if(!page->second->final) page->second->finalize();
    lock->leave();
}
...
unsigned findTotalPushBatchSize(asDWORD* nextOp, asDWORD* endOfBytecode)
{
    ...
    while(nextOp < endOfBytecode)
    {
        ...
        switch(op)
        {
            ...
            case asBC_PshV4:
            case asBC_PshG4:
            case asBC_TYPEID: bytes += sizeof(asDWORD); break;
            case asBC_PshV8:
            case asBC_PshC8: bytes += sizeof(asQWORD); break;
            case asBC_PSF:
            case asBC_PshVPtr:
            case asBC_PshRPtr:
            case asBC_PshNull:
            ...
            case asBC_PGA:
            case asBC_VAR:
            case asBC_PshGPtr: bytes += sizeof(void*); break;
...
asCScriptFunction* stdcall callInterfaceMethod(asIScriptContext* ctx, asCScriptFunction* func)
{
    ...
    if(context->m_status != asEXECUTION_ACTIVE) return 0;
...
asCScriptFunction* stdcall callBoundFunction(asIScriptContext* ctx, unsigned short fid)
{
    ...
    if(func->funcType != asFUNC_SCRIPT) return 0;
    context->CallScriptFunction(func);
    if(context->m_status != asEXECUTION_ACTIVE) return 0;
    return func;
}
...
bool stdcall doSuspend(asIScriptContext* ctx)
{
    ...
    if(Ctx->m_doSuspend)
    {
        ...
        if(Ctx->m_status == asEXECUTION_ACTIVE) Ctx->m_status = asEXECUTION_SUSPENDED;
        return true;
    }
    else
    {
...
#ifdef JIT_64
    if(sys->takesObjByVal || hasAutoHandles || sys->hostReturnSize > 4 || (sys->paramAutoHandles.GetLength() != 0 && sys->paramSize == 0))
#else
    if(sys->takesObjByVal || hasAutoHandles || sys->hostReturnSize > 2 || (sys->paramAutoHandles.GetLength() != 0 && sys->paramSize == 0))
#endif
    {
...
#ifdef JIT_64
            case ICC_CDECL:
            case ICC_STDCALL: call_64conv(sys, func, 0, OP_None); break;
            case ICC_CDECL_OBJLAST: call_64conv(sys, func, objPointer, OP_Last); break;
            case ICC_CDECL_OBJFIRST: call_64conv(sys, func, objPointer, OP_First); break;
            case ICC_THISCALL:
            case ICC_THISCALL_RETURNINMEM:
            case ICC_VIRTUAL_THISCALL:
            case ICC_VIRTUAL_THISCALL_RETURNINMEM:
    #ifdef _MSC_VER
                call_64conv(sys, func, objPointer, OP_This);
                break;
    #else
                call_64conv(sys, func, objPointer, OP_First);
                break;
    #endif
#else
            case ICC_CDECL: call_cdecl(sys, func); break;
            case ICC_STDCALL: call_stdcall(sys, func); break;
            case ICC_THISCALL:
            case ICC_THISCALL_RETURNINMEM: call_thiscall(sys, func, objPointer); break;
            case ICC_CDECL_OBJLAST: call_cdecl_obj(sys, func, objPointer, true); break;
            case ICC_CDECL_OBJFIRST: call_cdecl_obj(sys, func, objPointer, false); break;
            case ICC_VIRTUAL_THISCALL:
            case ICC_VIRTUAL_THISCALL_RETURNINMEM: call_viaAS(func, objPointer); break;
#endif
...
void SystemCall::call_entry(asSSystemFunctionInterface* func, asCScriptFunction* sFunc)
{
    ...
    if((flags & SC_FastFPU) == 0) fpu.init();
...
#ifdef JIT_64
void SystemCall::call_64conv(asSSystemFunctionInterface* func, asCScriptFunction* sFunc, Register* objPointer, ObjectPosition pos)
{
    ...
    if(sFunc->DoesReturnOnStack())
    {
        ...
        if(pos == OP_None || objPointer) arg0 = as<void*>(*esi);
        else
            arg0 = as<void*>(*esi + sizeof(asPWORD));
        if(acceptReturn) as<void*>(*esp + local::retPointer) = arg0;
        retPointer = true;
        argOffset += sizeof(void*);
    ...
    if(pos == OP_First)
    {
        if(!cpu.isIntArg64Register(firstPos, firstPos)) stackBytes += cpu.pushSize();
    ...
    for(; i < argCount; ++i, ++a)
    {
        ...
        {
            if(!cpu.isIntArg64Register(intCount, a)) stackBytes += cpu.pushSize();
            ++intCount;
            ++a;
            if(!cpu.isIntArg64Register(intCount, a)) stackBytes += cpu.pushSize();
            ++intCount;
        ...
        {
            if(!cpu.isFloatArg64Register(floatCount, a)) stackBytes += cpu.pushSize();
            ++floatCount;
    ...
    auto Arg = [&](Register* reg, bool dword) {
        if(dword) argOffset -= sizeof(asDWORD);
        else
            argOffset -= sizeof(asQWORD);

        if(reg)
        {
            if(dword) as<asDWORD>(*reg) = as<asDWORD>(*esi + argOffset);
            else
                as<asQWORD>(*reg) = as<asQWORD>(*esi + argOffset);
        }
        else
        {
            if(dword) cpu.push(as<asDWORD>(*esi + argOffset));
            else
    ...
    if(pos == OP_First && !cpu.isIntArg64Register(firstPos, firstPos)) cpu.push(temp);
    if(retPointer && !cpu.isIntArg64Register(0, 0) && func->hostReturnInMemory) cpu.push(pax);
    ...
    if(isVirtual)
    {
        ...
        if(containingObj.code == EAX) throw "Virtual function resolver doesn't know object register.";
    ...
    size_t addParams = 0;
    if(retPointer) addParams += sizeof(void*);
    if(stackObject) addParams += sizeof(void*);
    if(func->paramSize > 0 || addParams > 0) esi += func->paramSize * sizeof(asDWORD) + (unsigned)addParams;
    ...
    else if(func->hostReturnSize > 0 && acceptReturn)
    {
        ...
        {
            if(func->hostReturnSize == 1) as<uint32_t>(ebx) = as<uint32_t>(cpu.intReturn64());
            else if(func->hostReturnSize == 2)
...
void SystemCall::call_getReturn(asSSystemFunctionInterface* func, asCScriptFunction* sFunc)
{
    ...
    if(sFunc->returnType.IsObject() && !sFunc->returnType.IsReference())
    {
        if(sFunc->returnType.IsObjectHandle())
        {
            if(!acceptReturn)
            {
                if(func->returnAutoHandle) throw "Auto handle returns not permitted here.";  // Reference counting and deletion
                return;
            }
        ...
        else
        {
            ...
            if(!func->hostReturnInMemory)
            {
                if(func->hostReturnSize >= 1) *ecx = eax;
...
void SystemCall::call_stdcall(asSSystemFunctionInterface* func, asCScriptFunction* sFunc)
{
    ...
    {
        eax = as<void*>(*esi);
        if(acceptReturn) as<void*>(*esp + local::retPointer) = eax;
        lastArg += 1;
        popCount += sizeof(asDWORD);
    }

    for(int i = lastArg - 1; i >= firstArg; --i) cpu.push(*esi + (i * sizeof(asDWORD)));

    cpu.call((void*)func->func);

    if(popCount > 0) esi += popCount;
...
void SystemCall::call_cdecl_obj(asSSystemFunctionInterface* func, asCScriptFunction* sFunc, Register* objPointer, bool last)
{
    ...
    if(sFunc->DoesReturnOnStack())
    {
        ...
        if(acceptReturn) as<void*>(*esp + local::retPointer) = edx;
    ...
    if(!last)
    {
        if(objPointer) cpu.push(*objPointer);
        else
            cpu.push(ecx);
    }

    // retPointer is always last thing pushed
    if(sFunc->DoesReturnOnStack()) cpu.push(edx);
...
void SystemCall::call_thiscall(asSSystemFunctionInterface* func, asCScriptFunction* sFunc, Register* objPointer)
{
    ...
    unsigned popCount = func->paramSize * sizeof(asDWORD);
    if(popThis) popCount += sizeof(void*);
    if(sFunc->DoesReturnOnStack()) popCount += sizeof(void*);
    if(popCount > 0) esi += popCount;
...
void SystemCall::call_generic(asCScriptFunction* func, Register* objPointer)
{
    ...
    if(objPointer) arg2 = as<void*>(*objPointer);
    else
        arg2 ^= arg2;
    arg0          = as<void*>(*ebp + offsetof(asSVMRegisters, ctx));
    as<int>(arg1) = func->id;
...
void SystemCall::call_simple(Register& objPointer, asCScriptFunction* func)
{
    ...
    cpu.call_stdcall((void*)engineSimpleMethod, "prpp", (asCScriptEngine*)func->GetEngine(), &objPointer, sys, func);
}

void stdcall returnScriptFunction(asCContext* ctx)
{
    // Return if this was the first function, or a nested execution
    if(ctx->m_callStack.GetLength() == 0 || ctx->m_callStack[ctx->m_callStack.GetLength() - 9] == 0)
    {
