...
namespace assembler
{
    ...
    {
        if(address.absolute_address == 0) address.offset += 4;
        else
    ...
    void Processor::call_cdecl_prep(unsigned argBytes)
    {
        ...
        if(stackOffset != 0) esp -= 16 - stackOffset;
    }
    ...
    void Processor::call_cdecl_end(unsigned argBytes, bool returnPointer)
    {
        Register esp(*this, ESP, sizeof(void*) * 8);
        unsigned stackOffset = (stackDepth + argBytes) % 16;
        if(stackOffset != 0) argBytes += (16 - stackOffset);
#ifndef _MSC_VER
        if(returnPointer) argBytes -= 4;
#endif
        if(argBytes != 0) esp += argBytes;
    }
    ...
    unsigned Processor::call_thiscall_args(Register* obj, const char* args, va_list ap)
    {
        ...
        if(obj)
        {
            if(!isIntArg64Register(intCount, argCount)) stackBytes += pushSize();
            ++argCount;
            ++intCount;
            arg_stack.push(obj);
        }
        ...
        while(args && *args != '\0')
        {
            if(*args == 'r')
            {
                ...
                if(reg->xmm())
                {
                    if(!isFloatArg64Register(floatCount, argCount)) stackBytes += pushSize();
                    ++floatCount;
                }
                else
                {
...
#ifdef _MSC_VER
        if(stackBytes < 32) stackBytes = 32;
#endif
        ...
        while(!arg_stack.empty())
        {
            ...
            if(arg.reg)
            {
                ...
                if(reg.code == ESP || reg.code == R15) throw "Cannot use this register for cdecl call wrapper.";
                ...
                if(reg.xmm())
                {
                    if(isFloatArg64Register(floatA, a))
                    {
                        ...
                        {
                            if(used_regs.find(reg.code) != used_regs.end()) throw "Invalid out-of-order use of argument register.";
                            other = reg;
                        }
            ...
            else if(arg.mem)
            {
                ...
                if(argCount > 1)
                {
                    ...
                    {
                        if(addr.code == intArg64(i, i).code) throw "Cannot use this register for cdecl call wrapper address.";
                        if(addr.other == intArg64(i, i).code) throw "Cannot use this register for cdecl call wrapper address.";
                ...
                if(addr.code == R15 || addr.other == R15 || (addr.scaleFactor != 0 && addr.scaleReg == R15))
                    throw "Cannot use this register for cdecl call wrapper address.";
                if(addr.other == ESP) throw "Cannot use this register for cdecl call wrapper address.";
                if(addr.code == ESP)
                {
                    addr.code = R15;
    ...
    Processor& Processor::operator<<(MemAddress addr)
    {
        ...
        if(addr.scaleFactor != 0)
        {
            ...
            {
                case 1: scale = Scale1; break;
                case 2: scale = Scale2; break;
                case 4: scale = Scale4; break;
                case 8: scale = Scale8; break;
                default: throw 0;
            }

            // Can't scale with an offset from ESP
            if(addr.scaleReg % 8 == ESP) throw 0;
        ...
        {  //[EBP] means absolute address, so we use [EBP+0] instead
            *this << mod_rm(addr.other % 8, ADR, addr.code % 8);
            if(addr.code % 8 == ESP) *this << '\x24';  // SIB byte indicating [ESP]
        }
        else if(addr.offset >= CHAR_MIN && addr.offset <= CHAR_MAX)
        {
    ...
    Processor& Processor::operator<<(AddrPrefix pr)
    {
        ...
        if(pr.further != 0) *this << pr.further;

        // 64 bit prefix
        unsigned char ch = '\x40';
        if(!pr.defLong)
        {
            if(adr.bitMode == 64) ch |= '\x08';
        }
        if((adr.code % 16) > 7) ch |= '\x01';
        if((adr.other % 16) > 7) ch |= '\x04';
        if(adr.scaleReg > 7) ch |= '\x02';
        if(ch != '\x40') *this << ch;
        return *this;
    }
    ...
    Processor& Processor::operator<<(RegPrefix pr)
    {
        ...
        if(!pr.defLong)
        {
            if(reg.getBitMode() == 64) ch |= '\x08';
        }
        if((reg.code % 16) > 7) ch |= '\x01';
        if((other % 16) > 7) ch |= '\x04';
    ...
    {
        if(value <= CHAR_MAX) *this << '\x6A' << (byte)value;
        else
    ...
    void Processor::call(void* func)
    {
        ...
        if(offset < (int64_t)INT_MIN || offset > (int64_t)INT_MAX)
        {
            ...
            if(abs > (uint64_t)UINT_MAX) *this << '\x49' << '\xBB' << abs;
    ...
    unsigned char shortJumpCodes[JumpTypeCount] = { 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78,
                                                    0x79, 0x7A, 0x7B, 0x7C, 0x7D, 0x7E, 0x7F, 0xEB };
    unsigned char longJumpCodes[JumpTypeCount]  = { 0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88,
                                                   0x89, 0x8A, 0x8B, 0x8C, 0x8D, 0x8E, 0x8F, 0xE9 };
    ...
    void Processor::end_short_jump(void* p)
    {
        ...
        if(offset < CHAR_MIN || offset > CHAR_MAX) throw "Short jump too long.";
        *jumpFrom = (char)offset;
    }

    void* Processor::prep_long_jump(JumpType type)
    {
        if(type != Jump) *this << '\x0F';
    ...
    void Processor::end_long_jump(void* p)
    {
        ...
        if(offset < (int64_t)INT_MIN || offset > (int64_t)INT_MAX)
        {
            if(isSamePage) throw "Inside-page long jump too long. This should never ever happen, somebody screwed the pooch.";
            ...
            *this << reg.prefix(EX_4) << '\xFF' << reg.modrm(EX_4);
            ...
            if(offset < (int64_t)INT_MIN || offset > (int64_t)INT_MAX) throw "Multi-page drifting! Can't recover from this.";
        ...
        *(volatile int*)jumpFrom = (int)offset;
        if(isSamePage) jumpSpace -= 16;
    }

    void Processor::jump(JumpType type, volatile byte* dest)
    {
        int64_t offset = ((size_t)dest - (size_t)op) - 2;
        if(offset >= CHAR_MIN && offset <= CHAR_MAX) *this << shortJumpCodes[type] << (char)offset;
        else if(offset > INT_MAX || offset < INT_MIN)
        {
            ...
            *this << '\x0F' << longJumpCodes[type]
                  << (int)(offset - 4);  // Conditional long jump is 4 bytes larger, jump is from the end of the full opcode
    }
    ...
    void Processor::loop(volatile byte* dest, JumpType type)
    {
        ...
        if(off < CHAR_MIN || off > CHAR_MAX) throw "Loop offset too far.";

        if(type == Jump) *this << '\xE2' << (char)off;
        else if(type == Zero)
            *this << '\xE1' << (char)off;
    ...
    void MemAddress::operator++()
    {
        switch(bitMode)
        {
            case 8: cpu << prefix() << '\xFE' << *this; break;
            case 16: cpu << prefix('\x66') << '\xFF' << *this; break;
            default: cpu << prefix() << '\xFF' << *this; break;
        }
    }
    ...
    void MemAddress::operator-()
    {
        ...
        switch(bitMode)
        {
            case 8: cpu << prefix() << '\xF6' << *this; break;
            case 16: cpu << prefix('\x66') << '\xF7' << *this; break;
            default: cpu << prefix() << '\xF7' << *this; break;
        }
    }
    ...
    void MemAddress::operator=(unsigned int value)
    {
        switch(bitMode)
        {
            case 8: cpu << prefix() << '\xC6' << *this << (byte)value; break;
            case 16: cpu << prefix('\x66') << '\xC7' << *this << (unsigned short)value; break;
            case 32:
            default: cpu << prefix() << '\xC7' << *this << value; break;
        }
    }
    ...
    void MemAddress::operator|=(unsigned int value)
    {
        ...
        switch(bitMode)
        {
            case 8: cpu << prefix() << '\x80' << *this << (byte)value; break;
            case 16: cpu << prefix('\x66') << '\x81' << *this << (unsigned short)value; break;
            case 32:
            case 64: cpu << prefix() << '\x81' << *this << value; break;
        }
    }
    ...
    void MemAddress::operator=(Register fromReg)
    {
        ...
        if(fromReg.xmm())
        {
            switch(bitMode)
            {
                case 32: cpu << prefix('\xF3') << '\x0F' << '\x11' << *this; break;
                case 64: cpu << prefix('\xF2', true) << '\x0F' << '\x11' << *this; break;
                default: throw "Unsupported bitmode for xmm register";
            }
        }
        else
        {
            switch(bitMode)
            {
                case 8: cpu << prefix() << '\x88' << *this; break;
                case 16: cpu << prefix('\x66') << '\x89' << *this; break;
                default: cpu << prefix() << '\x89' << *this; break;
            }
        }
    }
    ...
    {
        if(bitMode != address.bitMode) throw 0;
    ...
    Register::Register(Processor& CPU, RegCode Code, unsigned BitModeOverride) : cpu(CPU), code(Code), bitMode(BitModeOverride) {}
    ...
    {
        if(bitMode == 0) return addr.bitMode;
        else
            return bitMode;
    }
    ...
    void Register::swap(MemAddress address)
    {
        ...
        {
            case 8: cpu << address.prefix() << '\x86' << address; break;
            case 16: cpu << address.prefix('\x66') << '\x87' << address; break;
            case 32:
            case 64: cpu << address.prefix() << '\x87' << address; break;
        }
    }
    ...
    void Register::swap(Register& other)
    {
        ...
        if(code == EAX) cpu << other.prefix() << byte(0x90 + (other.code % 8));
    ...
    {
        if(other.code != ECX) throw "Can only rotate by ECX";
    ...
    void Register::multiply_signed(MemAddress address, int value)
    {
        ...
        {
            if(value >= CHAR_MIN && value <= CHAR_MAX) cpu << address.prefix() << '\x6B' << address << (char)value;
            else
                cpu << address.prefix() << '\x69' << address << value;
        }
        else if(cpu.bitMode == 16)
        {
            if(value >= CHAR_MIN && value <= CHAR_MAX) cpu << address.prefix('\x66') << '\x6B' << address << (char)value;
            else
    ...
    void Register::operator--()
    {
        switch(getBitMode())
        {
            case 8: cpu << prefix(EX_1) << '\xFE' << modrm(EX_1); break;
            case 16: cpu << '\x66' << prefix(EX_1) << byte('\x48' + (code % 8)); break;
            default: cpu << prefix(EX_1) << '\xFF' << modrm(EX_1); break;
        }
    }
    ...
    void Register::operator++()
    {
        switch(getBitMode())
        {
            case 8: cpu << prefix() << '\xFE' << modrm(EX_0); break;
            case 16: cpu << '\x66' << prefix() << byte('\x40' + (code % 8)); break;
            default: cpu << prefix() << '\xFF' << modrm(EX_0); break;
        }
    }

    void Register::operator~()
    {
        if(getBitMode() == 8) cpu << prefix(EX_2) << '\xF6' << modrm(EX_2);
        else
    ...
    void Register::operator&=(unsigned long long mask)
    {
        switch(getBitMode())
        {
            case 8: cpu << prefix() << '\x80' << modrm(EX_4) << (byte)mask; break;
            case 16: cpu << prefix('\x66') << '\x81' << modrm(EX_4) << (unsigned short)mask; break;
            case 32:
            case 64:
                if(code == EAX) cpu << '\x25' << (unsigned)mask;
                else
    ...
    void Register::operator&=(Register other)
    {
        switch(getBitMode())
        {
            case 8: cpu << prefix(other) << '\x20' << modrm(other.code); break;
            case 16: cpu << '\x66' << prefix(other) << '\x21' << modrm(other.code); break;
            case 32:
            case 64: cpu << prefix(other) << '\x21' << modrm(other.code); break;
        }
    }
    ...
    void Register::operator|=(unsigned long long mask)
    {
        switch(getBitMode())
        {
            case 8: cpu << prefix() << '\x80' << modrm(EX_1) << (byte)mask; break;
            case 16: cpu << '\x66' << prefix() << '\x81' << modrm(EX_1) << (unsigned short)mask; break;
            case 32:
            case 64:
                if(code == EAX) cpu << '\x0D' << (unsigned)mask;
                else
    ...
    void Register::operator=(unsigned long long value)
    {
        switch(getBitMode())
        {
            case 8: cpu << prefix() << (byte)('\xB0' + (code % 8)) << (byte)value; break;
            case 16: cpu << '\x66' << prefix() << (byte)('\xB8' + (code % 8)) << (unsigned short)value; break;
            case 32: cpu << prefix() << (byte)('\xB8' + (code % 8)) << (unsigned int)value; break;
            case 64: cpu << prefix() << (byte)('\xB8' + (code % 8)) << value; break;
        }
    }
    ...
    void Register::operator=(Register other)
    {
        ...
        if(xmm())
        {
            if(bitMode == 0 && other.getBitMode() != getBitMode()) bitMode = other.getBitMode();
            switch(getBitMode())
            {
                case 32: cpu << '\xF3' << prefix(other) << '\x0F' << '\x10' << modrm(other); break;
                case 64: cpu << '\xF2' << prefix(other, true) << '\x0F' << '\x10' << modrm(other); break;
            }
        }
        else
        {
    ...
    void Register::copy_expanding(MemAddress address)
    {
        ...
        switch(getBitMode(address))
        {
            case 8: cpu << address.prefix() << '\x0F' << '\xBE' << address; break;
            case 16: cpu << address.prefix() << '\x0F' << '\xBF' << address; break;
            case 32: cpu << address.prefix() << '\x63' << address; break;
            case 64: *this = address; break;
        }
    }
    ...
    void Register::copy_zeroing(Register& other)
    {
        switch(getBitMode())
        {
            ...
            case 16: throw 0;
            case 32:
            case 64: cpu << other.prefix(*this) << '\x0F' << '\xB6' << other.modrm(code); break;
        }
    }
    ...
    void Register::operator=(MemAddress addr)
    {
        ...
        if(xmm())
        {
            if(bitMode == 0 && addr.bitMode != getBitMode()) bitMode = addr.bitMode;
            switch(addr.bitMode)
            {
                case 32: cpu << addr.prefix('\xF3') << '\x0F' << '\x10' << addr; break;
                case 64: cpu << addr.prefix('\xF2', true) << '\x0F' << '\x10' << addr; break;
            }
        }
        else
        {
            ...
            {
                case 8: cpu << addr.prefix() << '\x8A' << addr; break;
                case 16: cpu << addr.prefix('\x66') << '\x8B' << addr; break;
                default: cpu << addr.prefix() << '\x8B' << addr; break;
            }
        }
    }
    ...
    void Register::operator==(Register other)
    {
        switch(getBitMode())
        {
            case 8: cpu << other.prefix(*this) << '\x3A' << other.modrm(code); break;
            case 16: cpu << '\x66' << other.prefix(*this) << '\x3B' << other.modrm(code); break;
            case 32:
            case 64: cpu << other.prefix(*this) << '\x3B' << other.modrm(code); break;
        }
    }
    ...
    void Register::operator==(MemAddress addr)
    {
        ...
        switch(getBitMode(addr))
        {
            case 8: cpu << addr.prefix() << '\x3A' << addr; break;
            case 16: cpu << addr.prefix('\x66') << '\x3B' << addr; break;
            case 32:
            case 64: cpu << addr.prefix() << '\x3B' << addr; break;
        }
    }
    ...
    void Register::operator==(unsigned int test)
    {
        switch(getBitMode())
        {
            case 8: cpu << prefix(EX_7) << '\x80' << modrm(EX_7) << (byte)test; break;
            case 16: cpu << '\x66' << prefix(EX_7) << '\x80' << modrm(EX_7) << (unsigned short)test; break;
            case 32:
            case 64: cpu << prefix(EX_7) << '\x81' << modrm(EX_7) << test; break;
        }
    }

    unsigned char setConditions[JumpTypeCount - 1] = {
        0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x96, 0x97, 0x98, 0x99, 0x9A, 0x9B, 0x9C, 0x9D, 0x9E, 0x9F
    };

    void Register::setIf(JumpType condition)
    {
        if(condition >= Jump) throw 0;
    ...
    {
        if(pop) cpu << '\xDF' << mod_rm(EX_5, REG, floatReg);
        else
