...
namespace Crypto
{
    ...
    static std::string GetEncrypted(const std::string& input, const std::string& salt, uint32_t rounds = 1024)
    {
        ...
        if constexpr(std::is_same_v<Encryption, CryptoPP::PKCS5_PBKDF2_HMAC<CryptoPP::SHA256>> ||
            ...
            encryption.DeriveKey(
              digest, sizeof(digest), 0, (CryptoPP::byte*)input.c_str(), input.length(), (CryptoPP::byte*)salt.c_str(), salt.length(), rounds, 0.0f);
        else if constexpr(std::is_same_v<Encryption, CryptoPP::HKDF<CryptoPP::SHA256>>)
        {
            std::string info = std::to_string(rounds);
            encryption.DeriveKey(digest,
                                 sizeof(digest),
                                 (CryptoPP::byte*)input.c_str(),
                                 input.length(),
                                 (CryptoPP::byte*)salt.c_str(),
                                 salt.length(),
                                 (CryptoPP::byte*)info.c_str(),
                                 info.length());
        }
        else if constexpr(std::is_same_v<Encryption, CryptoPP::Scrypt>)
            encryption.DeriveKey(
              digest, sizeof(digest), (CryptoPP::byte*)input.c_str(), input.length(), (CryptoPP::byte*)salt.c_str(), salt.length(), rounds);
    ...
    #define ENCRYPTION_ALGORITHM_CASE(enumVal, algorithm) \
        case Crypto::EncryptAlgorithm::enumVal: return GetEncrypted<algorithm>(input, salt, rounds)
